...
const int im_h_idx = floor(threadlinidx/inp_w) % inp_h;
const int im_c_idx = floor(threadlinidx/(inp_w*inp_h) % filt_n;
const int im_b_idx = floor(threadlinidx/(inp_w*inp_h*filt_n))
...
int filt_idx = 0;
int j = 0;
for (int dh=0 ; dh< filt_h; dh++){
    isoutboundh = (dh + h) > inp_h;
    for ( int dw = 0 ; dw< filt_w; dw++ ){
        isoutbound = isoutboundh || ((dw + w) > inp_w);
        curand_init(seed,out_idx,dw + dh*(filt_w),&state);
        ...
        for  ( int chan = 0 ; chan < filt_c; chan++){
            ...
            this_px_out = this_px_out +  (flag2 * input[input_idx]); ////////// GL MEM ACESSS
            j = j + flag2;
            flag = false;
...
at::Tensor pad){
        ...
        const int blocks = ceil(totalOutpx/threadsperblock);

        const auto seed = time(NULL);
        AT_DISPATCH_FLOATING_TYPES(input.type(), "klconvs_forward_cuda", ([&] {
           klconvs_cuda_forward_kernel<scalar_t><<<blocks, threadsperblock>>>(seed,
              input.data<scalar_t>(),
