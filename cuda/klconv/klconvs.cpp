...
std::vector<at::Tensor> klconvs_cuda_backward_rand(at::Tensor grad_out, at::Tensor input,at::Tensor log_filt);
...
std::vector<at::Tensor> klconvs_backward_rand(at::Tensor grad_output,
at::Tensor input,
at::Tensor log_filt){
  ...
  CHECK_INPUT(grad_output);
    CHECK_INPUT(log_filt);
  return klconvs_cuda_backward_rand(grad_output, input, log_filt);

}
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &klconvs_forward, "klconvs forward");
  m.def("backward", &klconvs_backward, "klconvs backward");
  m.def("backward_rand", &klconvs_backward_rand, "klconvs backward with independent rand");
}